...
local function startFarmLoop()
    ...
    task.spawn(function()
        while autoFarm do
            -- Wait for round to become active before trying to farm
            local roundWaitStart = tick()
            while autoFarm and not isRoundActive() and not deadUntilNextRound do
                task.wait(0.5)
                -- safety: don't wait forever if map never loads
                if tick() - roundWaitStart > 90 then break end
            end
            ...
            if hrp and humanoid and humanoid.Health > 0 and isRoundActive() and not deadUntilNextRound and not waitingForNewMap then
                if farmMode == "Safe" then
                    safe2FarmMain(hrp)
                else
                    normalFarmMain(hrp)
                end
            elseif deadUntilNextRound then
                -- Already handled at top of loop; just wait
                task.wait(0.5)
            else
                task.wait(0.5)
            end
