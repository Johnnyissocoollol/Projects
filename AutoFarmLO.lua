...
-- Shared helper: called whenever fling loop finishes (for any reason)
-- to ensure auto reset / lobby TP always happens.
local function finishAfterFling()
    hasFlingedThisRound = true
    flingLoopActive = false
    if autoResetEnabled then
        if not deadUntilNextRound then
...
-- Skips friends and self. Auto reset (or lobby TP) always fires after.
local function startFlingLoop()
    ...
    task.spawn(function()
        ...
        -- No murderer found: skip fling, still do reset
        if not murderer then
            finishAfterFling()
            return
        end

        -- We ARE the murderer: skip fling, just reset
        if murderer == localplayer then
            finishAfterFling()
            return
        end

        -- Murderer is a friend: skip fling, still do reset
        ...
        while flingMurderEnabled and not isFlung(murderer) do
            ...
            if not char or not humanoid or humanoid.Health <= 0 then
                -- Dead mid-fling: wait to respawn, then keep going
...
local function handleRoundEnd(hrp)
    ...
    if flingMurderEnabled and not hasFlingedThisRound then
        -- fling loop always calls finishAfterFling() which handles auto reset / lobby TP
        startFlingLoop()
