...
local flingRetriesLeft = 0        -- how many retries remain after a death mid-fling
local flingTargetName = nil       -- username of the target we're retrying against
local safe2FirstCoin = true -- true at round start; first coin TPs underground before tween

-- ========== SKIDFLING FUNCTION ==========
local function isMurderer(player)
    if not player or not player.Character then return false end
    return (player.Backpack and player.Backpack:FindFirstChild("Knife")) ~= nil
        or (player.Character and player.Character:FindFirstChild("Knife")) ~= nil
end

local function localPlayerIsMurderer()
    return isMurderer(localplayer)
end
...
local function findMurderer(allowFriends)
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= localplayer then
            if not allowFriends and isFriend(player) then continue end
...
-- finds murderer regardless of friend status (for friend-check purposes)
local function findAnyMurderer()
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= localplayer then
            if (player.Backpack and player.Backpack:FindFirstChild("Knife")) or 
               (player.Character and player.Character:FindFirstChild("Knife")) then
...
-- Returns true if the fling visibly launched the target (velocity spiked)
local function flingSucceeded(targetPlayer)
    local tchar = targetPlayer and targetPlayer.Character
    if not tchar then return false end
    local thrp = tchar:FindFirstChild("HumanoidRootPart")
    if not thrp then return false end
    return thrp.Velocity.Magnitude > 500
end

-- Core fling executor: attempts up to `retries` times, returns true if succeeded.
-- Called both at round-end and after respawn (retry path).
local function executeFling(murderer, retries)
    if not murderer or not murderer.Character then return false end
    if not murderer.Character:FindFirstChildWhichIsA("BasePart") then return false end

    getgenv().OldPos = nil

    -- Stable fling position in lobby
    ...
    for attempt = 1, retries do
        ...
        task.wait(0.25)
        if flingSucceeded(murderer) then
            return true
        end
...
local function flingMurdererAtRoundEnd()
    ...
    hasFlingedThisRound = true  -- prevent double-trigger

    task.spawn(function()
        -- If WE are the murderer, skip fling entirely and just auto-reset/lobby
        if localPlayerIsMurderer() then
        ...
        -- Find ANY murderer (friend or not) so we can decide what to do
        local anyMurderer = nil
        local deadline = tick() + 5
        repeat
            task.wait(0.3)
            anyMurderer = findAnyMurderer()
            -- Also re-check: if we became murderer mid-search, bail out
            if localPlayerIsMurderer() then
                if autoResetEnabled then
                    deadUntilNextRound = true
                    killCharacter()
        ...
        until anyMurderer or tick() > deadline

        if not anyMurderer then
            -- No murderer found — fall through to normal auto-reset/lobby handling
            if autoResetEnabled then
                deadUntilNextRound = true
                killCharacter()
            else
        ...
        -- Murderer IS our friend — skip the fling, handle normally
        if isFriend(anyMurderer) then
            if autoResetEnabled then
                deadUntilNextRound = true
                killCharacter()
            else
        ...
        -- Murderer is NOT a friend — fling them (up to 5 attempts)
        flingTargetName = anyMurderer.Name
        flingRetriesLeft = 5  -- remaining retries if we die mid-fling

        local succeeded = executeFling(anyMurderer, 5)

        -- Whether fling worked or not, now do the normal round-end action
        flingRetriesLeft = 0
        flingTargetName = nil
        if autoResetEnabled then
            deadUntilNextRound = true
...
local function doSafe2Farm(hrp)
    ...
    if coin and isCoinValid(coin) then
        ...
        -- On the very first coin of the round, teleport directly underground
        -- so we start from the right Y instead of falling through from spawn height
        if safe2FirstCoin then
            safe2FirstCoin = false
            enableNoclip()
            ...
            disableNoclip()
            anchorHRP(hrp, true)
            return -- let the main loop pick this coin again on next tick with a normal tween
        end
...
local function handleRoundEnd(hrp)
    ...
    if flingMurderEnabled and not hasFlingedThisRound then
        -- flingMurdererAtRoundEnd is fully async and owns the auto-reset/lobby call
        -- after it finishes, so we must NOT also call killCharacter/teleportToLobby here.
        flingMurdererAtRoundEnd()
        return
    end

    -- No fling — handle round-end normally
    if autoResetEnabled then
        deadUntilNextRound = true
        killCharacter()
    else
...
local function normalFarmMain(hrp)
    ...
    while autoFarm and not deadUntilNextRound and not waitingForNewMap do
        if not isRoundActive() or allCoinsGone() then
            noCoinsStreak = noCoinsStreak + 1
            if noCoinsStreak >= 3 then  -- must fail 3 consecutive checks (~0.6s) before ending
                handleRoundEnd(hrp)
                break
            end
            task.wait(0.2)
...
local function safe2FarmMain(hrp)
    if not hrp or waitingForNewMap then return end
    safe2FirstCoin = true -- reset so first coin of this round TPs underground
    anchorHRP(hrp, false)
    ...
    while autoFarm and not deadUntilNextRound and not waitingForNewMap do
        ...
        if not isRoundActive() or allCoinsGone() then
            noCoinsStreak = noCoinsStreak + 1
            if noCoinsStreak >= 3 then  -- must fail 3 consecutive checks (~0.15s) before ending
                handleRoundEnd(hrp)
                break
            end
        else
...
local function customDeathHandler()
    ...
    safe2FirstCoin = true
    removeInvisibleFloor()
    -- Do NOT reset hasFlingedThisRound here if we still have retries pending —
    -- onCharacterAdded will continue the fling after respawn.
    if flingRetriesLeft <= 0 then
        hasFlingedThisRound = false
    end
    getgenv().OldPos = nil
    ...
    task.spawn(function()
        while autoFarm do
            if deadUntilNextRound then
                -- Wait for respawn, but don't get stuck if char already loaded
                local waitStart = tick()
                if not localplayer.Character or not localplayer.Character:FindFirstChild("HumanoidRootPart") 
                    or localplayer.Character:FindFirstChild("Humanoid") and localplayer.Character.Humanoid.Health <= 0 then
                    -- Character not ready yet — wait for CharacterAdded with a timeout
                    local charAddedEvent = localplayer.CharacterAdded
                    local newChar = nil
                    local conn
                    conn = charAddedEvent:Connect(function(c)
                        newChar = c
                        conn:Disconnect()
                    end)
                    -- Timeout after 30s in case we miss the event
                    local timeout = tick() + 30
                    while not newChar and tick() < timeout do
                        task.wait(0.1)
                    end
                    if conn then pcall(function() conn:Disconnect() end) end
                end
                task.wait(2)
                if not getActiveMap() then
                    waitForNewMapToLoad()
                else
                    hasFlingedThisRound = false
                    flingRetriesLeft = 0
            ...
            if char and not deadUntilNextRound and not waitingForNewMap then
                ...
                if hrp and humanoid and humanoid.Health > 0 then
                    if isRoundActive() then
                        ...
                        -- Not active yet — wait briefly then re-check before giving up
                        task.wait(0.5)
                        if autoFarm and not deadUntilNextRound and not waitingForNewMap then
                            if not isRoundActive() then
                                -- Double-check: wait a little more for round to start
                                task.wait(1)
                            end
                        end
                    end
...
local function onCharacterAdded()
    ...
    -- If we were killed mid-fling, resume retrying against the same target
    if flingMurderEnabled and flingRetriesLeft > 0 and flingTargetName then
        task.spawn(function()
            task.wait(1.5) -- let character fully load
            local retriesThisLife = flingRetriesLeft
            flingRetriesLeft = 0

            local murderer = Players:FindFirstChild(flingTargetName)
            if murderer and murderer.Character then
                executeFling(murderer, retriesThisLife)
            end

            flingTargetName = nil
            hasFlingedThisRound = false

            -- Round is over after retries. Let startFarmLoop handle the respawn cycle
            -- by going through the normal deadUntilNextRound path.
            -- If auto reset is off, just go to lobby; startFarmLoop will pick up next round.
            if not autoResetEnabled then
                deadUntilNextRound = false
                teleportToLobby()
            end
            -- If autoResetEnabled, deadUntilNextRound is already true from customDeathHandler,
            -- and startFarmLoop's CharacterAdded wait already fired for this respawn,
            -- so we just let it loop naturally on the next round.
        end)
        return  -- don't run the block below; startFarmLoop owns the rest
    end

    -- Normal respawn after auto-reset (no fling retries pending)
...
modeRow.Position = UDim2.new(0, 14, 0, 204)
...
speedRow.Position = UDim2.new(0, 14, 0, 245)
