    ...
    task.spawn(function()
        ...
        while flingMurderEnabled and flingAttempts < MAX_FLING_ATTEMPTS do
            ...
            if not murderer then
                -- no murderer found at all, bail
                break
            end

            -- if murderer is a friend → skip fling, auto reset instead
            ...
            if not murderer.Character or not murderer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- start watching for airborne so we can kill murder the instant they fly
                ...
                if weDied then
                    ...
                    -- fling completed without dying → done
                    break
                end
            end
        end

        -- exhausted all attempts → reset and wait for next round
        if flingAttempts >= MAX_FLING_ATTEMPTS then
            deadUntilNextRound = true
            if autoResetEnabled then
                killCharacter()
            end
...
print("Script loaded - Fling Murder: friend check, 6-attempt retry, airborne kill, murderer self-reset")