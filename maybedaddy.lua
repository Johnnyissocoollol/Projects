...
local function doNormalFarm(hrp)
    ...
    -- Loop: re-pick nearest coin and tween to it, redirecting if it disappears mid-tween
    while autoFarm and not deadUntilNextRound and not waitingForNewMap do
        coin = findNearestCoin(hrp)
        if not coin or not isCoinValid(coin) then break end
        ...
        -- Watch for the coin disappearing mid-tween (collected by MTF or another player)
        ...
        while not tweenDone do
            ...
            if not isCoinValid(coin) then
                -- Target coin gone: cancel and re-route to next nearest
                cancelActiveTween()
                tweenDone = true -- exit inner wait, outer loop will pick new coin
            end
        end

        -- If coin is still valid we reached it — we're done for this call
        if isCoinValid(coin) then
            break
        end
        -- Otherwise loop again to reroute
    end
...
local function doSafe2Farm(hrp)
    ...
    -- Loop: keep picking nearest coin and redirect if it vanishes mid-tween
    while autoFarm and not deadUntilNextRound and not waitingForNewMap do
        local coin = findNearestCoin(hrp)
        if not coin or not isCoinValid(coin) then break end
        ...
        if tweenTime > 0 then
            ...
            -- Watch for coin disappearing mid-tween
            local tweenDone = false
            activeTween.Completed:Connect(function() tweenDone = true end)
            ...
            while not tweenDone do
                task.wait(0.05)
                if not isCoinValid(coin) then
                    -- Coin gone: cancel tween and reroute
                    cancelActiveTween()
                    tweenDone = true
                end
            end
        ...
        -- Coin was collected mid-tween, loop will pick next nearest
        if not isCoinValid(coin) then
            anchorHRP(hrp, true)
            continue
        end

        -- Coin still valid: do the pickup
        ...
        hrp.CFrame = CFrame.new(deepPos)
        anchorHRP(hrp, true)
        break -- successfully collected this coin, return to caller
    end
end
