...
local function flingMurdererAtRoundEnd()
    ...
    hasFlingedThisRound = true -- set immediately to prevent double-trigger

    task.spawn(function()
        -- clear stale OldPos so SkidFling doesn't warp us to a wrong position
        getgenv().OldPos = nil
        ...
        -- teleport us to lobby for a stable fling position
        ...
        -- run fling in pcall so errors don't break future rounds
...
local function setupNoclip()
    ...
    local heartbeatConn = RunService.Heartbeat:Connect(function()
        if autoFarm and char and char.Parent then
            for _, part in ipairs(char:GetDescendants()) do
                if part:IsA("BasePart") and part.CanCollide then
                    part.CanCollide = false
                end
...
local function moveInvisibleFloor(x, y, z)
    if fakeFloor then fakeFloor.CFrame = CFrame.new(x, y - 3, z) end
end
...
local function doNormalFarm(hrp)
    ...
    -- mark visited immediately so other iterations skip it
    visitedCoins[coin] = true
    ...
    flyConn = RunService.Heartbeat:Connect(function()
        ...
        -- abort if coin was collected mid-flight by someone else
        if not isCoinValid(coin) then
            bv.Velocity = Vector3.zero
            flyConn:Disconnect()
            return
        end
        local dir = coinPos - hrp.Position
    ...
    local timeout = tick() + math.max(dist / flySpeed, 0.05) + 2
    repeat task.wait() until not flyConn.Connected or tick() > timeout
    if flyConn.Connected then flyConn:Disconnect() end
...
local function safe2FarmMain(hrp)
    if not hrp or waitingForNewMap then return end
    anchorHRP(hrp, false)
    local startPos = Vector3.new(hrp.Position.X, hrp.Position.Y + safe2UndergroundOffset - 2, hrp.Position.Z)
    createInvisibleFloor(startPos.Y - 3)
...
local function onCharacterAdded()
    ...
    if autoResetEnabled and deadUntilNextRound then
        task.spawn(function()
            ...
            task.wait(1)
            teleportToMap()
            if autoFarm then setupNoclip() end
        end)
    end
end
...
titleLabel.Text = "Auto Farm + Fling Murder"
...
local modes = { "Safe", "Normal", "Lobby TP" }
local modeIndex = 1
