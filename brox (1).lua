...
-- Use a deterministic per-account offset so multiple accounts spread across
-- different coins instead of all racing to the same nearest coin.
-- Coins are sorted by a position-based key, then each account picks from
-- a staggered index based on its UserId mod (number of coins).
local function findNearestCoin(hrp)
    local coinContainer = findActiveCoinContainer()
    if not coinContainer then return nil end

    -- Collect all valid, unvisited coins
    local available = {}
    for _, coin in ipairs(coinContainer:GetChildren()) do
        if coin:IsA("BasePart") and coin.Name == "Coin_Server"
            and coin:FindFirstChild("CoinVisual") and not visitedCoins[coin] then
            table.insert(available, coin)
        end
    end
    if #available == 0 then return nil end

    -- Sort coins by a stable spatial key so all accounts see the same ordering
    table.sort(available, function(a, b)
        local ax = math.floor(a.Position.X * 10)
        local bx = math.floor(b.Position.X * 10)
        if ax ~= bx then return ax < bx end
        local az = math.floor(a.Position.Z * 10)
        local bz = math.floor(b.Position.Z * 10)
        return az < bz
    end)

    -- Each account starts searching from a different offset in the sorted list,
    -- spreading accounts across different coins.
    local offset = localplayer.UserId % #available
    -- Among candidates starting at the offset, pick the closest one
    local nearest, bestDist = nil, math.huge
    for i = 0, #available - 1 do
        local idx = (i + offset) % #available + 1
        local coin = available[idx]
        local dist = (hrp.Position - coin.Position).Magnitude
        -- Bias: prefer coins in "our" partition (first half of the staggered list)
        -- by adding a distance penalty for coins outside our share
        local penalty = (i >= math.ceil(#available / math.max(Players.LocalPlayer and 1 or 1, 1))) and 50 or 0
        if dist + penalty < bestDist then
...
local function doNormalFarm(hrp)
    ...
    -- Monitor mid-tween: cancel immediately if the coin gets collected by someone else
    local tweenDone = false
    activeTween.Completed:Connect(function() tweenDone = true end)
    repeat
        task.wait(0.05)
        if not isCoinValid(coin) then
            -- Coin gone — abort tween early, coin stays in visitedCoins so we skip it
            cancelActiveTween()
            tweenDone = true
        end
    until tweenDone
...
local function doSafe2Farm(hrp)
    ...
    if coin and isCoinValid(coin) then
        ...
        if tweenTime > 0 then
            ...
            activeTween:Play()
            -- Cancel early if coin gets snatched by another account mid-tween
            local tweenDone = false
            activeTween.Completed:Connect(function() tweenDone = true end)
            repeat
                task.wait(0.05)
...
local function handleRoundEnd(hrp)
    ...
    -- Only attempt to fling if WE are NOT the murderer
    if flingMurderEnabled and not hasFlingedThisRound and not isLocalPlayerMurderer() then
        flingMurdererAtRoundEnd()
    end

    if autoResetEnabled then
        -- If we're the murderer, just reset cleanly - no fling needed
        deadUntilNextRound = true
        killCharacter()
    else
    ...
    task.spawn(function()
        while autoFarm do
            ...
            if deadUntilNextRound then
                ...
                -- Snap to wait spot and lock immediately on respawn
                if char then
                    local hrp = char:FindFirstChild("HumanoidRootPart")
                    if hrp then
            ...
            -- ── PRE-ROUND: lock at wait spot until coins appear (all modes) ──
            hrp.CFrame = CFrame.new(-28.3, 338.6, -115.7)
            task.wait(0.1)
            lockInLobby()
            ...
            while autoFarm and not isRoundActive() and not deadUntilNextRound do
                task.wait(0.1)
                -- re-lock if character respawned mid-wait (lobbyLockConnection gone = new character)
                if not lobbyLockConnection then
            ...
            -- unlock the instant coins appear
            unlockFromLobby()
...
local function onCharacterAdded()
    ...
    if char then
        ...
        -- Always snap to wait spot if autoFarm is on and no coins are up yet
        if autoFarm and not isRoundActive() then
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then
                hrp.CFrame = CFrame.new(-28.3, 338.6, -115.7)
                task.wait(0.05)
