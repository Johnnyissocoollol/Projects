...
local airborneKillTriggered = false  -- true when the murderer was successfully launched
...
local function startAirborneKillWatch(murderer)
    ...
    airborneWatchConn = RunService.Heartbeat:Connect(function()
        ...
        if hrp and hrp.Velocity.Y > 200 then
            -- they're in the air — kill them immediately
            airborneKillTriggered = true  -- mark as successful launch
            stopAirborneWatch()
...
local function tryFlingMurderer()
    ...
    -- successful fling so that all 6 retries can actually run if early
    -- attempts fail (character missing, no murderer found, we die, etc.)

    task.spawn(function()
        flingAttempts = 0
        local flingSucceeded = false

        while flingMurderEnabled and flingAttempts < MAX_FLING_ATTEMPTS do
            flingAttempts = flingAttempts + 1

            -- find murderer — search up to 5s per attempt
            local murderer = nil
            ...
            if not murderer then
                -- no murderer visible this attempt — keep retrying
                task.wait(0.5)
                -- do NOT break; loop back and try again up to MAX_FLING_ATTEMPTS
            elseif isFriend(murderer) then
                -- murderer is a friend → never fling friends, bail entirely
                hasFlingedThisRound = true  -- prevent any further attempt this round
                if autoResetEnabled then
                    deadUntilNextRound = true
                    killCharacter()
                end
                break
            elseif not murderer.Character or not murderer.Character:FindFirstChild("HumanoidRootPart") then
                -- murderer found but character not ready yet — short wait then retry
                task.wait(0.5)
            else
                -- murderer found and character ready — attempt the fling
                getgenv().OldPos = nil
                local char = localplayer.Character
                ...
                -- If the airborne watch fired, the murderer was successfully launched
                if airborneKillTriggered then
                    flingSucceeded = true
                ...
                elseif weDied then
                    ...
                    -- fling ran to completion without us dying → success
                    flingSucceeded = true
                    hasFlingedThisRound = true
                    break
                end
        ...
        -- exhausted all 6 attempts without success
        if not flingSucceeded and flingAttempts >= MAX_FLING_ATTEMPTS then
            hasFlingedThisRound = true  -- stop retrying this round
            deadUntilNextRound = true
            if autoResetEnabled then
                killCharacter()
            end
...
local function doWalkFarm(hrp)
    ...
    -- Walk to the underground position
    humanoid:MoveTo(deepPos)

    -- Wait for MoveToFinished or 15s timeout
    local arrived = false
    local moveConn
    ...
    repeat task.wait(0.05) until arrived or tick() > timeout or deadUntilNextRound or waitingForNewMap
...
print("Script loaded - Fling Murder: friend check, no retry on successful airborne launch, airborne kill, anti-fling toggle")